...
import { iForm, iFormDataCopy } from "../constants/interfaces";
...
const EditListing = (): JSX.Element => {
  ...
  //     navigate("/");
  //   }
  // }, [auth.currentUser?.uid, listing, navigate]);    FIX LATER
  ...
  const onSubmitHandler: (e: React.FormEvent<HTMLFormElement>) => void = async (
    ...
    const imgUrls = await Promise.all(
      [...images].map((image: any) => storeImage(image))
    ).catch((error) => {
      setLoading(false);
